...
def get_noll_indices(args):
    ...
    minclude = np.array(args.include_4pi, dtype=np.int)
    mexclude = np.array(args.exclude_4pi, dtype=np.int)
...
NB: DO NOT USE SPACES in this list!''')
    parser.add_argument(
        '--include-4pi', type=str, default=None, metavar='INDICES',
        help='''
...
NB: DO NOT USE SPACES in this list!''')
    parser.add_argument(
        '--noll-min', type=int, help='Minimum Zernike Noll index to consider')
    parser.add_argument(
        '--noll-max', type=int, help='Minimum Zernike Noll index to consider')
